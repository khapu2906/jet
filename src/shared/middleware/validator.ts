 ...
 */
export function validate(schema: v.GenericSchema, source: ValidationSource = 'body') {
 ...
 */
export function validateMultiple(validations: Array<{ schema: v.GenericSchema; source: ValidationSource }>) {
  return async (c: Context, next: Next) => {
    ...
    for (const validation of validations) {
      try {
        ...
        if (error instanceof v.ValiError) {
          const details = error.issues.map((e) => ({
            field: `${validation.source}.${e.path?.map(p => p.key).join('.') || ''}`,
            message: e.message,
          }));
