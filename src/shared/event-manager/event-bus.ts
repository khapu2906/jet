...
export abstract class CoreEventBus implements EventBus {
	...
	constructor(protected config: EventBusConfig) {
		...
		this._log(
			`${this.constructor.name} instance #${this.instanceId} created (role: ${this.role})`,
		);
	}
	...
	async publish(event: DomainEvent): Promise<void> {
		...
		if (this.role === "consumer") {
			throw new Error(
				'Consumer-only EventBus cannot publish events. Set EVENT_BUS_ROLE to "both" or "publisher".',
			);
		}
	...
	subscribe(handler: EventHandler): void {
		...
		if (this.role === "publisher") {
			this._log(
				`Skipping handler subscription (role: publisher): ${handler.eventName}`,
			);
			return;
		}
		...
		this._log(
			`Instance #${this.instanceId} subscribing to event: ${eventName} with handler: ${handler.constructor.name}`,
		);

		// Validate event name if config.events is not '*'
		if (this.config.events !== "*" && !this.config.events.includes(eventName)) {
		...
		this._log(
			`Total handlers for ${eventName}: ${this.handlers.get(eventName)!.length}`,
		);
	...
	protected _onHandlerSubscribed(
		_eventName: string,
	...
	protected async _executeHandlers(
		...
		this._log(
			`Processing event ${event.name} with ${handlers.length} handler(s)`,
		);
		...
		const results = await Promise.allSettled(
			handlers.map(async (handler) => {
				this._log(
					`Executing handler: ${handler.constructor.name} for event ${event.name}`,
				);
				try {
					await handler.handle(event);
					this._log(
						`Handler ${handler.constructor.name} completed successfully`,
					);
					return {
					...
					this._error(
						`Handler ${handler.constructor.name} failed for ${event.name}: ${handlerError}`,
					);
					return {
						handler: handler.constructor.name,
		...
		this._log(
			`Event ${event.name} processed: ${successCount} success, ${failureCount} failed`,
		);
	}
