...
To run multiple worker consumers, start multiple `PROCESS_TYPE=worker` instances via PM2, Docker Compose, or Kubernetes. PgBoss distributes jobs across all running instances without duplication.
